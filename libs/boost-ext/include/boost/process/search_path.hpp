 ...
 *
 * \a filename must be a basename including the file extension.
 ...
 * Windows to find \a filename.
 *
 * \a path must be a set of directories. Directories must be
 * separated by colons on POSIX and by semicolons on Windows.
 * If \a path is empty, the environment variable PATH is used.
 *
 * \returns the absolute path to the executable \a filename or an
 *          empty string if \a filename isn't found
 *
 * \throws std::runtime_error if \a path is empty and no environment
